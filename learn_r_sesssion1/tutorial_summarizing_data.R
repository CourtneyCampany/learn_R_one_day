...
traits_agg3 <- summaryBy(chl_mg_m2 ~ niche, FUN=mean, data=traits_nona,
                         keep.names = TRUE)
...
str(traits_summ)
fernsumm <- traits_summ[, -c(3, 9)]

write.csv(fernsumm, "working_data/fern_traits_summary.csv", row.names = FALSE)
...
#inspect the data and use the appropirate variables to merge

...
# try with traits: plot frond length by lamina length:

plot()

#make a boxplot of frond length by niche
boxplot()

#make a histogram of frond length
hist()
...
plot(frond_length_cm ~ lamina_area_cm2, data=traits,
     ylim=c(), xlim=c()) #alter axis ranges

# easily add treatment colors as long as they are factors

plot(frond_length_cm ~ lamina_area_cm2, data=traits,
     ylim=c(), xlim=c(),
     pch=, bg=) #choose a symbol type and color (col or bg) by factor 'niche'

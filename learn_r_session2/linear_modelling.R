...
# lets examine a plant trait by habitat (niche)

boxplot(stipe_length_cm ~ niche, data=traits)
#looks like there are possible some differences and a few outliers 
# lets test using lm()

stipe_mod <- lm(stipe_length_cm ~ niche, data=traits)
stipe_mod
# here are coefficients are our contrasts with the intercept as the mean 1st variable
...
anova(stipe_mod) # we can run ust the anova with the built in function

#there is a Anova() function in the car package
library(car)
?Anova() #it lets you change your between type II or II sums of squares
...
stipe_mod2 <- lm(stipe_length_cm ~ niche + species, data=traits)
...
anova(stipe_mod,stipe_mod3)
#compare AIC, pick the model with lowest AIC
AIC(stipe_mod,stipe_mod3)
